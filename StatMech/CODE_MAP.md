...
Root facade: `StatMech.lean` (imports all layers).
Layer facades: `StatMech/Hamiltonian.lean`, `StatMech/MeanField.lean`, `StatMech/ContinuumField.lean`, `StatMech/Consensus.lean`.
Stochastic facade: `StatMech/Hamiltonian/Stochastic.lean` (re-exports `Basic`, `LangevinFokkerPlanck`, `ChannelNoise`).
...
Defines the StatMech/Boltzmann ensemble and the ergodic hypothesis. The partition function normalizes the Boltzmann weight exp(−V/kT) into a probability density; ergodicity asserts that time averages along trajectories converge to ensemble averages against this density.
...
Compatibility bridge that aliases Telltale `Protocol.Spatial` types (`Site`, `RoleName`, `SpatialReq`, `Topology`, `Satisfies`) into the StatMech continuum namespace, plus `satisfiesBool` reflection.
