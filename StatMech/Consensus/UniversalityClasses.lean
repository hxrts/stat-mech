...
abbrev UniversalityClass : Type := StatMech.MeanField.UniversalityClass
...
def classOf (hasGap : Prop) (hasTunneling : Prop) : UniversalityClass := by
  ...
  exact StatMech.MeanField.classOf hasGap hasTunneling

/-- A consensus system is gapped if it has a positive energy gap. -/
def IsGapped {Exec : Type} [StatMech.Hamiltonian.EnergyDistance Exec]
    (A B : Set Exec) : Prop := by
  -- Use the physics-first gap notion.
  exact StatMech.Hamiltonian.HasEnergyGap A B
