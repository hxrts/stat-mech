...
This file collects all axioms in the StatMech library. Every other result in the
...
The Hamiltonian facade (`StatMech.Hamiltonian`) imports this file, making the
axioms available to downstream modules. They are used only in
`StatMech.Consensus.ChannelThreshold` to prove `codingSafe_iff_positive_gap`,
...
axiom channel_coding_achievability {X Y : Type*} [Fintype X] [Fintype Y]
    (W : StatMech.Hamiltonian.Channel.DMC X Y) (R ε : ℝ)
    (hR : R < StatMech.Hamiltonian.Channel.channelCapacity W) (hε : 0 < ε) :
    ∃ n₀ : ℕ, ∀ n ≥ n₀,
      ∃ (M : Type) (_ : Fintype M)
