...
`StatMech.ContinuumField` now aliases Telltale's canonical `Protocol.Spatial`
definitions directly. This removes local drift and keeps StatMech aligned with the
