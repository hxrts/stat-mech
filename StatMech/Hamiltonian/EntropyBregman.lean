...
namespace StatMech.Hamiltonian.EntropyBregman
...
theorem kl_eq_bregman_negEntropy (n : ℕ) (p q : Fin n → ℝ)
    ...
    StatMech.Hamiltonian.Entropy.klDivergence p q =
  ...
  unfold StatMech.Hamiltonian.Entropy.klDivergence bregman
...
theorem kl_nonneg_via_bregman (n : ℕ) (p q : Fin n → ℝ)
    ...
    (hq_pos : ∀ i, 0 < q i) (hq_sum : ∑ i, q i = 1) :
    0 ≤ StatMech.Hamiltonian.Entropy.klDivergence p q :=
  StatMech.Hamiltonian.Entropy.klDivergence_nonneg p q hp_nn hp_sum
...
theorem legendre_negEntropy_eq_logSumExp (n : ℕ) [NeZero n] (θ : Config n) :
    StatMech.Hamiltonian.legendreOn (negEntropyConfig n) (simplex n) θ =
      Real.log (∑ i : Fin n, Real.exp ((fromConfig θ) i)) := by
...
theorem freeEnergy_eq_scaled_legendre_dual (n : ℕ) [NeZero n]
    ...
    PartitionFunction.freeEnergy H β =
      -(1/β) * StatMech.Hamiltonian.legendreOn (negEntropyConfig n) (simplex n)
...
end StatMech.Hamiltonian.EntropyBregman
