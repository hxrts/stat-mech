...
namespace StatMech.Hamiltonian.Channel
...
theorem jointDist_marginalFst {X Y : Type*} [Fintype X] [Fintype Y]
    (W : DMC X Y) (p : X → ℝ) :
    StatMech.Hamiltonian.Entropy.marginalFst (jointDist W p) = p := by
  funext x
  calc
    StatMech.Hamiltonian.Entropy.marginalFst (jointDist W p) x
...
theorem jointDist_marginalSnd {X Y : Type*} [Fintype X] [Fintype Y]
    (W : DMC X Y) (p : X → ℝ) :
    StatMech.Hamiltonian.Entropy.marginalSnd (jointDist W p) = outputDist W p := by
  funext y
  calc
    StatMech.Hamiltonian.Entropy.marginalSnd (jointDist W p) y
...
def channelMutualInfo {X Y : Type*} [Fintype X] [Fintype Y]
    (W : DMC X Y) (p : X → ℝ) : ℝ :=
  StatMech.Hamiltonian.Entropy.mutualInfo (jointDist W p)

/-- Channel capacity C(W) = sup_p I(p;W). -/
def channelCapacity {X Y : Type*} [Fintype X] [Fintype Y] (W : DMC X Y) : ℝ :=
  ⨆ (d : StatMech.Hamiltonian.Entropy.Distribution X), channelMutualInfo W d.pmf
...
theorem channelCapacity_nonneg {X Y : Type*} [Fintype X] [Fintype Y]
  ...
  have h_joint_nonneg (d : StatMech.Hamiltonian.Entropy.Distribution X) :
      ∀ ab, 0 ≤ jointDist W d.pmf ab := by
    rintro ⟨x, y⟩
    exact mul_nonneg (d.nonneg x) (W.transition_nonneg x y)
  have h_joint_sum (d : StatMech.Hamiltonian.Entropy.Distribution X) :
    ...
    calc
      ...
      _ = 1 := d.sum_one
  have h_output_nonneg (d : StatMech.Hamiltonian.Entropy.Distribution X) :
      ∀ y, 0 ≤ outputDist W d.pmf y := by
    intro y
    refine Finset.sum_nonneg ?_
    intro x hx
    exact mul_nonneg (d.nonneg x) (W.transition_nonneg x y)
  have h_output_sum (d : StatMech.Hamiltonian.Entropy.Distribution X) :
    ...
    calc
      ...
      _ = 1 := h_joint_sum d
  have h_bound (d : StatMech.Hamiltonian.Entropy.Distribution X) :
      channelMutualInfo W d.pmf ≤
        Real.log (Fintype.card X) + Real.log (Fintype.card Y) := by
    have hHx :
        StatMech.Hamiltonian.Entropy.shannonEntropy d.pmf ≤
          Real.log (Fintype.card X) := by
      exact StatMech.Hamiltonian.Entropy.shannonEntropy_le_log_card d.pmf d.nonneg d.sum_one
    have hHy :
        StatMech.Hamiltonian.Entropy.shannonEntropy (outputDist W d.pmf) ≤
          Real.log (Fintype.card Y) := by
      exact StatMech.Hamiltonian.Entropy.shannonEntropy_le_log_card
        (outputDist W d.pmf) (h_output_nonneg d) (h_output_sum d)
    have hHxy :
        0 ≤ StatMech.Hamiltonian.Entropy.shannonEntropy (jointDist W d.pmf) :=
      StatMech.Hamiltonian.Entropy.shannonEntropy_nonneg (jointDist W d.pmf)
        (h_joint_nonneg d) (h_joint_sum d)
    unfold channelMutualInfo StatMech.Hamiltonian.Entropy.mutualInfo
    have hle1 :
        StatMech.Hamiltonian.Entropy.shannonEntropy (StatMech.Hamiltonian.Entropy.marginalFst (jointDist W d.pmf)) +
            StatMech.Hamiltonian.Entropy.shannonEntropy (StatMech.Hamiltonian.Entropy.marginalSnd (jointDist W d.pmf)) -
            StatMech.Hamiltonian.Entropy.shannonEntropy (jointDist W d.pmf)
          ≤
        StatMech.Hamiltonian.Entropy.shannonEntropy (StatMech.Hamiltonian.Entropy.marginalFst (jointDist W d.pmf)) +
            StatMech.Hamiltonian.Entropy.shannonEntropy (StatMech.Hamiltonian.Entropy.marginalSnd (jointDist W d.pmf)) := by
      linarith
    have hle2 :
        StatMech.Hamiltonian.Entropy.shannonEntropy (StatMech.Hamiltonian.Entropy.marginalFst (jointDist W d.pmf)) +
            StatMech.Hamiltonian.Entropy.shannonEntropy (StatMech.Hamiltonian.Entropy.marginalSnd (jointDist W d.pmf))
      ...
      have hfst : StatMech.Hamiltonian.Entropy.marginalFst (jointDist W d.pmf) = d.pmf :=
        jointDist_marginalFst W d.pmf
      have hsnd : StatMech.Hamiltonian.Entropy.marginalSnd (jointDist W d.pmf) = outputDist W d.pmf :=
        jointDist_marginalSnd W d.pmf
      simpa [hfst, hsnd] using add_le_add hHx hHy
    exact le_trans hle1 hle2
  have hB : BddAbove (Set.range (fun d : StatMech.Hamiltonian.Entropy.Distribution X =>
  ...
  let d0 : StatMech.Hamiltonian.Entropy.Distribution X :=
    { pmf := fun x => if x = x0 then 1 else 0
      ...
      sum_one := by
        simp }
  have hmi : 0 ≤ StatMech.Hamiltonian.Entropy.mutualInfo (jointDist W d0.pmf) :=
    StatMech.Hamiltonian.Entropy.mutualInfo_nonneg (jointDist W d0.pmf)
...
private theorem mutualInfo_le_shannonEntropy_marginalFst {X Y : Type*}
    ...
    StatMech.Hamiltonian.Entropy.mutualInfo pXY ≤
      StatMech.Hamiltonian.Entropy.shannonEntropy
        (StatMech.Hamiltonian.Entropy.marginalFst pXY) := by
  -- condEntropy_nonneg gives 0 ≤ H(X,Y) - H(Y), i.e. H(Y) ≤ H(X,Y)
  have hcond := StatMech.Hamiltonian.Entropy.condEntropy_nonneg pXY h_nn h_sum
  -- mutual info = H(X) + H(Y) - H(X,Y), so I ≤ H(X) iff H(Y) ≤ H(X,Y)
  unfold StatMech.Hamiltonian.Entropy.mutualInfo StatMech.Hamiltonian.Entropy.condEntropy at *
  linarith
...
private theorem mutualInfo_le_shannonEntropy_marginalSnd {X Y : Type*}
    ...
    StatMech.Hamiltonian.Entropy.mutualInfo pXY ≤
      StatMech.Hamiltonian.Entropy.shannonEntropy
        (StatMech.Hamiltonian.Entropy.marginalSnd pXY) := by
  -- use symmetry: I(X;Y) = I(Y;X) ≤ H(margFst(pYX)) = H(margSnd(pXY))
  have hsymm := StatMech.Hamiltonian.Entropy.mutualInfo_symm pXY
  ...
  have hfst_eq : StatMech.Hamiltonian.Entropy.marginalFst pYX =
      StatMech.Hamiltonian.Entropy.marginalSnd pXY := by
    funext y; simp [StatMech.Hamiltonian.Entropy.marginalFst,
      StatMech.Hamiltonian.Entropy.marginalSnd, pYX]
  rw [hfst_eq] at hbound
  -- mutualInfo pXY = mutualInfo pYX by symmetry
  have : StatMech.Hamiltonian.Entropy.mutualInfo pXY =
      StatMech.Hamiltonian.Entropy.mutualInfo pYX := by
    exact hsymm
  linarith
...
theorem channelCapacity_le_log_output {X Y : Type*} [Fintype X] [Fintype Y]
  ...
  have h_bound (d : StatMech.Hamiltonian.Entropy.Distribution X) :
    ...
    rw [hsnd] at hI_le
    -- H(output) ≤ log |Y|
    have hH_le := StatMech.Hamiltonian.Entropy.shannonEntropy_le_log_card
      (outputDist W d.pmf) (h_output_nonneg d) (h_output_sum d)
    exact le_trans hI_le hH_le
  -- witness for Nonempty (Distribution X) needed by ciSup_le
  classical
  obtain ⟨x0⟩ := (inferInstance : Nonempty X)
  haveI : Nonempty (StatMech.Hamiltonian.Entropy.Distribution X) :=
    ⟨{ pmf := fun x => if x = x0 then 1 else 0
...
private theorem dmc_joint_entropy_eq {X Y : Type*} [Fintype X] [Fintype Y]
    ...
    StatMech.Hamiltonian.Entropy.shannonEntropy (jointDist W p) =
      StatMech.Hamiltonian.Entropy.shannonEntropy p +
      ∑ x, p x *
        StatMech.Hamiltonian.Entropy.shannonEntropy (fun y => W.transition x y) := by
  classical
  unfold StatMech.Hamiltonian.Entropy.shannonEntropy jointDist
...
private theorem dmc_mutualInfo_eq {X Y : Type*} [Fintype X] [Fintype Y]
    (W : DMC X Y) (d : StatMech.Hamiltonian.Entropy.Distribution X) :
    channelMutualInfo W d.pmf =
      StatMech.Hamiltonian.Entropy.shannonEntropy (outputDist W d.pmf) -
      ∑ x, d.pmf x *
        StatMech.Hamiltonian.Entropy.shannonEntropy (fun y => W.transition x y) := by
...
theorem capacity_as_free_energy_dual {X Y : Type*} [Fintype X] [Fintype Y]
    [Nonempty X] [Nonempty Y] (W : DMC X Y) :
    channelCapacity W =
      ⨆ (d : StatMech.Hamiltonian.Entropy.Distribution X),
        StatMech.Hamiltonian.Entropy.shannonEntropy (outputDist W d.pmf) -
        ∑ x, d.pmf x *
          StatMech.Hamiltonian.Entropy.shannonEntropy (fun y => W.transition x y) := by
...
private lemma bsc_row_entropy (ε : ℝ) (hε₀ : 0 ≤ ε) (hε₁ : ε ≤ 1)
    ...
    StatMech.Hamiltonian.Entropy.binaryEntropy ε := by
  classical
  unfold StatMech.Hamiltonian.Entropy.shannonEntropy
    StatMech.Hamiltonian.Entropy.binaryEntropy BSC
...
private lemma bsc_condEntropy_sum (ε : ℝ) (hε₀ : 0 ≤ ε) (hε₁ : ε ≤ 1)
    (d : StatMech.Hamiltonian.Entropy.Distribution Bool) :
    ∑ x, d.pmf x *
      StatMech.Hamiltonian.Entropy.shannonEntropy
        (fun y => (BSC ε hε₀ hε₁).transition x y) =
    StatMech.Hamiltonian.Entropy.binaryEntropy ε := by
...
private lemma shannonEntropy_uniform_bool :
    StatMech.Hamiltonian.Entropy.shannonEntropy (fun _ : Bool => (1 : ℝ)/2) =
      Real.log 2 := by
  unfold StatMech.Hamiltonian.Entropy.shannonEntropy
...
theorem bsc_capacity (ε : ℝ) (hε₀ : 0 ≤ ε) (hε₁ : ε ≤ 1) :
    channelCapacity (BSC ε hε₀ hε₁) =
      Real.log 2 - StatMech.Hamiltonian.Entropy.binaryEntropy ε := by
  classical
  ...
  have hsimpl (d : StatMech.Hamiltonian.Entropy.Distribution Bool) :
      StatMech.Hamiltonian.Entropy.shannonEntropy
          (outputDist (BSC ε hε₀ hε₁) d.pmf) -
        ∑ x, d.pmf x * StatMech.Hamiltonian.Entropy.shannonEntropy
          (fun y => (BSC ε hε₀ hε₁).transition x y) =
      StatMech.Hamiltonian.Entropy.shannonEntropy
          (outputDist (BSC ε hε₀ hε₁) d.pmf) -
        StatMech.Hamiltonian.Entropy.binaryEntropy ε := by
  ...
  haveI : Nonempty (StatMech.Hamiltonian.Entropy.Distribution Bool) :=
  ...
  · -- upper bound: H(output) ≤ log |Bool| = log 2
    ...
    have hle := StatMech.Hamiltonian.Entropy.shannonEntropy_le_log_card
      (outputDist (BSC ε hε₀ hε₁) d.pmf)
  ...
  · -- lower bound: uniform input achieves log 2
    let d0 : StatMech.Hamiltonian.Entropy.Distribution Bool :=
      { pmf := fun _ => 1/2
    ...
    have hbdd : BddAbove (Set.range fun d : StatMech.Hamiltonian.Entropy.Distribution Bool =>
        StatMech.Hamiltonian.Entropy.shannonEntropy
          (outputDist (BSC ε hε₀ hε₁) d.pmf) -
        StatMech.Hamiltonian.Entropy.binaryEntropy ε) := by
      refine ⟨Real.log 2 - StatMech.Hamiltonian.Entropy.binaryEntropy ε, ?_⟩
      rintro _ ⟨d', rfl⟩
      have hle := StatMech.Hamiltonian.Entropy.shannonEntropy_le_log_card
        (outputDist (BSC ε hε₀ hε₁) d'.pmf)
        (bsc_output_nonneg ε hε₀ hε₁ d') (bsc_output_sum ε hε₀ hε₁ d')
...
theorem bsc_capacity_half :
  ...
  rw [bsc_capacity]
  unfold StatMech.Hamiltonian.Entropy.binaryEntropy
...
end StatMech.Hamiltonian.Channel
