...
import StatMech.Hamiltonian.Stochastic.Basic
...
noncomputable def langevinSDE (H : ConvexHamiltonian n) (d : Damping) (σ : ℝ) :
    StatMech.Hamiltonian.Stochastic.SDE n := by
...
noncomputable def langevinProcess {Ω : Type*} [MeasurableSpace Ω]
    ...
    (W : StatMech.Hamiltonian.Stochastic.BrownianMotion (Ω := Ω) n)
    (X : StatMech.Hamiltonian.Stochastic.StochasticProcess (Ω := Ω) n)
    (hX : StatMech.Hamiltonian.Stochastic.SolvesSDE (Ω := Ω) (langevinSDE H d σ) W X) :
    StatMech.Hamiltonian.Stochastic.SDEProcess (Ω := Ω) n :=
